...
| 監査対象操作 (success) | `connect disconnect mkdirat unlinkat renameat fchmod fchown` |
| 監査対象操作 (failure) | `connect` のみ (拒否された接続) |
| FileServer | 有効 (`ENABLE_AUDIT="yes"`) |
| BackupServer | **無効 (`ENABLE_AUDIT="no"`)** — Time Machine の rename/unlink 大量発生対策 |
...
CloudWatch Agent の `app/config/cloudwatch_agent/FileServer.conf` で `LOG_PATHS` に `/var/log/samba/audit.log:smb-audit` を追加し、ロググループ `/onprem/FileServer` のストリーム `smb-audit` に転送する。
...
| BackupServer も同条件で audit ON | Time Machine の差分生成は rename/unlink を毎秒数十件発生させる。1 バックアップで数十万件、CW Logs ingestion 料金とノイズが容認できない。Backup の挙動は Mac OS 側の Time Machine ログで追跡可能 |
...
- BackupServer はファイル単位の操作が監査対象から外れる。Time Machine の不正利用検知は別レイヤー (NAS 接続元 IP の制限、Time Machine 側暗号化、CW メトリクスの容量推移) に依存する
...
- `app/setup/samba.sh`: `ENABLE_AUDIT` フラグ追加、`vfs objects` 動的組立、smb.conf に `full_audit:*` 注入、rsyslog drop-in / logrotate 配置ステップを追加。OFF 切替時はクリーンアップ
- `app/config/samba/{FileServer,BackupServer,sample}.conf`: `ENABLE_AUDIT` 行追加 (FileServer=yes, BackupServer=no, sample=no)
...
- `app/config/cloudwatch_agent/FileServer.conf`: `LOG_PATHS` に `/var/log/samba/audit.log:smb-audit` 追加
- BackupServer の CloudWatch Agent 設定は変更なし
